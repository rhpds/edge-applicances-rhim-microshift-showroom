...
| Multi-stage deployment complexity | **Single artifact delivery:** One disk image deploys the full system.
|===
...
[TIP]
.Production Recommendation: ISO with Kickstart
====
For **production deployments**, ISO is typically the preferred format because:

* **Platform-independent:** Works on any hardware—bare metal, VMs, or edge devices—without hypervisor-specific drivers.
* **Zero-touch deployment:** Attach a **Kickstart** file to automate the entire installation process with no manual intervention.
* **Portable delivery:** Ship via USB drives, PXE boot servers, or any standard installation workflow.
* **Universal compatibility:** No dependency on specific virtualization platforms or cloud providers.

To generate an ISO instead of QCOW2, simply change `--type qcow2` to `--type iso` in the `bootc-image-builder` command.

The Kickstart file uses the `ostreecontainer` directive to pull the OS directly from the embedded OCI archive on the ISO—enabling fully offline, automated installations at remote sites.
====
