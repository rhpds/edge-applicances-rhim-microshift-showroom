...
[cols="1a,1a", options="header"]
|===
| Traditional Appliance Lifecycle | Image Mode (Technical Workflow)
...
|
1. *Describe:* Define appliance in a `Containerfile`
2. *Build:* Create bootable image using `bootc`
3. *Generate:* Produce bootable ISO/disk image
4. *Deploy:* Install to VM or physical edge device
5. *Update:* Push a new tagged image for atomic upgrade
|===
